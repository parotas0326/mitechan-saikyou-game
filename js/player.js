...
Mite.Player = class {
 constructor(stage){
  ...
  this.kickLatch=false;this.punchLatch=false;this.specialLatch=false;
 ...
 update(dt,input,stage){
  ...
  if(this.attack===0&&this.comboGrace===0&&this.attackKind!=='kick')this.comboStep=0;
  const direction=(this.hitstun>0||this.special>0||this.attackKind==='kick'&&this.attack>0)?0:Number(input.down('right'))-Number(input.down('left'));
  ...
  const punchHeld=input.down('punch'), kickHeld=input.down('kick');
  if(!punchHeld)this.punchLatch=false;if(!kickHeld)this.kickLatch=false;
  if(this.special>0){this.special=Math.max(0,this.special-dt);this.attack=0;this.attackQueued=false;this.attackKind='none';}
   ...
   if(this.hitstun<=0&&this.attack===0&&kickHeld&&!this.kickLatch){this.startKick();this.kickLatch=true;}
   else if(this.hitstun<=0&&this.attack===0&&punchHeld){this.startPunch();this.punchLatch=true;}
   if(this.hitstun<=0&&this.attack===0&&this.attackQueued){this.startPunch();this.attackQueued=false;}
  }

  this.state=this.hitstun>0?'hurt':this.special>0?'special':this.attackKind==='kick'&&this.attack>0?'kick':this.attack>0?'punch':this.y<stage.floor?'jump':direction?'walk':'idle';
  if(this.state==='punch'||this.state==='special')this.frame=this.attack>.20||this.special>.44?'punch0':'punch1';
  else if(this.state==='kick')this.frame='walk2';
 ...
 startKick(){
  this.attackKind='kick';this.comboStep=0;this.comboGrace=0;this.attack=.44;this.attackId++;this.attackQueued=false;Mite.SFX?.punch(3);
 }
 ...
 healFull(){this.hp=this.maxHp;this.power=0;this.dead=false;this.invuln=0;this.hitstun=0;this.attack=0;this.attackQueued=false;this.attackKind='none';this.jumpLatch=false;this.special=0;this.comboStep=0;this.comboGrace=0;this.kickLatch=false;this.specialLatch=false;}
 get body(){return{x:this.x-20,y:this.y-90,width:40,height:90};}
 get hitbox(){
  if(this.special>0)return null;
  if(this.attackKind==='kick'&&this.attack>.11&&this.attack<.31)return{x:this.facing>0?this.x+18:this.x-88,y:this.y-60,width:70,height:30};
 ...
 draw(ctx,images,stage){
  ...
  if(this.state==='kick'&&this.attack>0){
    ctx.save();ctx.translate(-7,-2);ctx.rotate(-.08);ctx.drawImage(images[this.frame],-52,-120,128,128);ctx.restore();
    const active=this.attack<.32&&this.attack>.10;
    if(active){ctx.fillStyle='#f2c53f';ctx.fillRect(32,-48,54,13);ctx.fillStyle='#fff7db';ctx.fillRect(76,-51,27,17);ctx.fillStyle='#24324a';ctx.fillRect(93,-48,12,13);}
  }else ctx.drawImage(images[this.frame],-52,-120,128,128);
