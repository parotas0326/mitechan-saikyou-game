...
Mite.Player = class {
 constructor(stage){
  ...
  this.comboStep=0;this.comboGrace=0;this.power=0;this.maxPower=100;this.special=0;this.specialLatch=false;
 }
 update(dt,input,stage){
  ...
  if(this.hitstun<=0&&this.special<=0&&jumpHeld&&!this.jumpLatch&&this.y>=stage.floor){this.vy=-520;this.jumpLatch=true;Mite.SFX?.jump();}
  if(!jumpHeld)this.jumpLatch=false;
  this.vy+=1450*dt;this.y+=this.vy*dt;stage.constrain(this);

  if(this.special>0){this.special=Math.max(0,this.special-dt);this.attack=0;this.attackQueued=false;}
  else{
   const punchHeld=input.down('punch');
   if(this.attack>0){this.attack=Math.max(0,this.attack-dt);if(punchHeld&&this.attack<.10)this.attackQueued=true;}
   if(this.hitstun<=0&&punchHeld&&this.attack===0){this.startPunch();}
 ...
 startPunch(){
  this.comboStep=this.comboGrace>0?(this.comboStep%3)+1:1;
 ...
 startSpecial(){this.power=0;this.special=.72;this.attack=0;this.attackQueued=false;this.comboStep=0;this.attackId++;Mite.SFX?.special();}
 hurt(amount,fromX){
  if(this.invuln>0||this.dead)return false;
  this.hp=Math.max(0,this.hp-amount);this.invuln=.82;this.hitstun=.24;this.attack=0;this.attackQueued=false;this.special=0;
  this.x+=this.x<fromX?-28:28;Mite.SFX?.hurt();
  if(this.hp<=0)this.dead=true;
  return true;
 }
 healFull(){this.hp=this.maxHp;this.power=0;this.dead=false;this.invuln=0;this.hitstun=0;this.attack=0;this.attackQueued=false;this.jumpLatch=false;this.special=0;this.comboStep=0;this.comboGrace=0;}
 get body(){return{x:this.x-20,y:this.y-90,width:40,height:90};}
 get hitbox(){
  if(this.special>0)return null;
  if(this.attack>.055&&this.attack<(this.comboStep===3?.26:.205))return{x:this.facing>0?this.x+18:this.x-68,y:this.y-84,width:this.comboStep===3?58:50,height:28};
  return null;
 }
 draw(ctx,images,stage){
  ...
  if(this.comboStep===3&&this.attack>.08&&this.attack<.28){ctx.fillStyle='#fff2a4';ctx.fillRect(52,-82,14,5);ctx.fillStyle='#ff9f31';ctx.fillRect(62,-86,11,13);}
  ctx.restore();
 }
};
