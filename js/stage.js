...
Mite.Stage = class {
 ...
 configure(number,background){this.number=number;this.background=background;this.width=number===2?3500:3400;this.cameraX=0;this.lockX=null;}
 ...
 draw(ctx){ctx.fillStyle='#101827';ctx.fillRect(0,0,640,360);const cam=this.cameraX;if(this.background){const tileW=this.number===2?640:960;for(let x=0;x<this.width;x+=tileW){const sx=Math.round(x-cam);if(sx>640||sx+tileW<0)continue;ctx.drawImage(this.background,sx,0,tileW,360);}}
 ...
 else{ctx.fillStyle='rgba(52,36,24,.34)';ctx.fillRect(0,this.floor,640,60);ctx.fillStyle='#c8894c';ctx.fillRect(0,this.floor,640,3);ctx.font='bold 11px monospace';ctx.fillStyle='rgba(255,240,220,.78)';ctx.fillText('STAGE 2  戦場',18,24);}
 }
